...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    monodepth = None
    depth_path = os.path.join(args.source_path, "depths", cam_info.image_name + ".png")
    if os.path.exists(depth_path):
        depth_image = cv2.imread(depth_path, cv2.IMREAD_UNCHANGED)
        if depth_image is not None:
            if depth_image.ndim == 3:
                depth_image = depth_image[:, :, 0]
            monodepth = depth_image.astype(np.float32) / 65535.0
    ...
    return Camera(resolution, colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T,
                  ...
                  monodepth=monodepth, mask=mask, bounds=cam_info.bounds,
                  focalx=cam_info.focalx, focaly=cam_info.focaly,
