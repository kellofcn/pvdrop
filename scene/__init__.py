    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            scene_info = sceneLoadTypeCallbacks["Colmap"](
                args.source_path, args.images, args.eval, args.n_views, rand_pcd=args.rand_pcd)
        ...
        for resolution_scale in resolution_scales:
            ...
            if len(train_views) > 0:
                has_bounds = all(getattr(view, "bounds", None) is not None for view in train_views)
                if (args.source_path.find('llff') != -1 or args.source_path.find('DTU') != -1) and has_bounds:
                    pseudo_poses = generate_random_poses_llff(train_views)
                else:
                    pseudo_poses = generate_random_poses_360(train_views)
