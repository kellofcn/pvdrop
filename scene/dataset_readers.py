...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, path, rgb_mapping):
    cam_infos = []
    rgb_lookup = {}
    for image_file in rgb_mapping:
        basename = os.path.basename(image_file)
        stem = os.path.splitext(basename)[0]
        rgb_lookup[basename.lower()] = image_file
        rgb_lookup[stem.lower()] = image_file
    skipped = 0
    ...
    for idx, key in enumerate(sorted(cam_extrinsics.keys())):
        ...
        rgb_path = rgb_lookup.get(image_basename.lower(), rgb_lookup.get(image_stem.lower()))
        if rgb_path is None and os.path.exists(image_path):
            rgb_path = image_path
        if rgb_path is None:
            skipped += 1
            continue
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=rgb_path,
                image_name=image_name, width=width, height=height, mask=None, bounds=bounds,
                focalx=focal_length_x, focaly=focal_length_y)
        cam_infos.append(cam_info)

    sys.stdout.write('\n')
    if skipped > 0:
        print(f"[Warning] Skipped {skipped} COLMAP cameras without matching images in {images_folder}.")
    if len(cam_infos) == 0:
        raise RuntimeError(f"No COLMAP cameras matched images in {images_folder}.")
    return cam_infos


def list_image_files(images_folder):
    image_extensions = {".jpg", ".jpeg", ".png"}
    return [
        f for f in sorted(glob.glob(os.path.join(images_folder, '*')))
...
def readColmapSceneInfo(path, images, eval, n_views=0, llffhold=8, rand_pcd=False):
    ...
    rgb_mapping = list_image_files(images_folder)
    cam_extrinsics = {cam_extrinsics[k].name: cam_extrinsics[k] for k in cam_extrinsics}
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics,
                             images_folder=images_folder,  path=path, rgb_mapping=rgb_mapping)
