...
)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			atomicAdd(&dL_dmean2D[global_id].x, tmp_x);
			atomicAdd(&dL_dmean2D[global_id].y, tmp_y);
			atomicAdd(&dL_dmean2D[global_id].z, fabs(tmp_x));
			atomicAdd(&dL_dmean2D[global_id].w, fabs(tmp_y));
